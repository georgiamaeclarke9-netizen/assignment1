...
class Student:
    ...
    def __init__(
        ...
        *,
        # feature knobs
        n_lags=5,                 # r_{t-1} ... r_{t-n}
        mom_windows=(5, 10, 20),  # mean of daily log-returns
        vol_window=20,             # std of daily log-returns
        sma_windows=(10, 20),    
        ema_windows=(8, 16),
        rsi_window=10,
    ...
    def fit(self, X_train: pd.DataFrame, y_train: pd.Series, meta=None):
        ...
        for a in self.alpha_grid:
            ...
            for tr_idx, va_idx in tscv.split(F.values):
                ...
                pipe = Pipeline([("scaler", StandardScaler()), ("model", ElasticNet(alpha=a, l1_ratio=0.5, random_state=self.random_state))])
                pipe.fit(X_tr, y_tr)
        ...
        self.pipe_ = Pipeline([("scaler", StandardScaler()), ("model", ElasticNet(alpha=self.best_alpha_, l1_ratio=0.5, 
